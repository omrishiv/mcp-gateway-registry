...
Both are wired across all three deployment surfaces (Docker `.env`, Terraform/ECS `update_check_enabled` / `update_check_interval_hours`, and Helm `registry.app.updateCheck.enabled` / `.intervalHours`). See [docs/unified-parameter-reference.md](../docs/unified-parameter-reference.md) Group 13c.
