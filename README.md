...
- **ARD Catalog Publisher (`/.well-known/ai-catalog.json`)** - The registry now publishes its public assets as an [Agentic Resource Discovery (ARD)](https://github.com/ards-project/ard-spec) catalog, making it a conformant **ARD Catalog Publisher** (spec v1.0). A new public, no-auth endpoint renders every public + enabled MCP server, A2A agent, and skill as an ARD `catalogEntry` with a domain-anchored URN (`urn:air:<publisher>:<namespace>:<name>`), the correct IANA media type per asset (`application/mcp-server-card+json`, `application/a2a-agent-card+json`, `application/ai-skill`), derived `representativeQueries` and `capabilities`, and an `https` `trustManifest` for publisher identity. Entry `url`s point at dedicated anonymous per-record endpoints under `/api/public/*` that return only public + enabled records and strip internal fields. The rendered manifest passes the ARD `conformance-test manifest` CLI with zero critical errors. This is a format adapter over existing data (no new storage), configurable via `ARD_CATALOG_ENABLED` / `ARD_PUBLISHER_DOMAIN` and wired across Docker, Terraform/ECS, and Helm. Phase 1 of 3 (Publisher); search and federation are tracked separately. ([#1294](https://github.com/agentic-community/mcp-gateway-registry/issues/1294), [#1299](https://github.com/agentic-community/mcp-gateway-registry/pull/1299)) [ARD Catalog Publisher Guide](docs/ard-catalog-publisher.md)
