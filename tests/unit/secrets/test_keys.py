...
The correctness-critical guard from the LLD: an Auth0 ``sub`` (``auth0|abc``),
a Keycloak username with a space/accent, and a multi-segment ``server_path``
must all encode to separator-free segments so they cannot collide with the
``/`` (OpenBao path) or ``|`` (map-key) delimiters, and must round-trip
identically.
"""
