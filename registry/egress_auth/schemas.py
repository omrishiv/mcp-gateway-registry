...
class EgressAuthMode(str, Enum):
    ...
    NONE = "none"  # no egress auth (default; today's behavior)
    OAUTH_USER = "oauth_user"  # per-user 3LO token from the vault (this feature)


class TokenEndpointAuthStyle(str, Enum):
    """Where the client_secret goes when calling the provider token endpoint."""

    POST_BODY = "post_body"  # client_id/client_secret in the form body (default)
