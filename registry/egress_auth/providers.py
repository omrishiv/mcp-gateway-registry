...
Adding a provider is a config row here, not a new module (the AgentCore
"one engine + config table" model). The ~3 providers that bend the spec use a
small ``token_response_parser`` hook in ``oauth_engine`` rather than bespoke
code.
