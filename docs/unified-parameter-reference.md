...
| CIMD consumer enabled | `CIMD_CONSUMER_ENABLED` | `cimd_consumer_enabled` | `registry.app.cimdConsumerEnabled` | Advertise client_id_metadata_document_supported in the registry's RFC 8414 AS metadata so MCP clients present a CIMD URL as client_id to the upstream IdP; the IdP performs the fetch/validate. Default off; enable only when the configured IdP supports CIMD (issue #993). |
