# MCP Gateway Observability Guide (LEGACY — pre-1.25.0 architecture)

> **OBSOLETE — for reference only.**
>
> This document describes the pre-1.25.0 observability architecture, where
> registry/auth-server/mcpgw POSTed JSON metric events to a centralized
> ``metrics-service`` container that persisted them to SQLite and exposed a
> Prometheus exporter on port 9465. That architecture was retired in
> [#1122](https://github.com/agentic-community/mcp-gateway-registry/issues/1122);
> services now emit metrics natively via the OpenTelemetry SDK in-process.
>
> **For the current architecture, see [OBSERVABILITY.md](OBSERVABILITY.md).**
>
> This document is retained for one release (1.25.0) so operators upgrading
> from 1.24.x can cross-reference the old SQLite-based queries and the
> ``METRICS_LEGACY_HTTP_POST=true`` transition flag. It will be deleted in
> 1.26.0 along with the metrics-service container itself.
